      ...
      <div className="rounded-[4px] m-2 p-2">
        {mockLabelsData.map((el) => {
          ...
          return (
            <div
              key={el.key}
              className="flex justify-between items-center hover:bg-gray-100 rounded-[4px] p-2"
            >
              <div key={el.key} className="text-gray-600">
                {el.description}
              </div>
              <div key={el.key} className={labelStyle}></div>
            </div>
          );
        })}
      </div>
